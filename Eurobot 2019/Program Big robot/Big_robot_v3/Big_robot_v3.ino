...
void loop() { //********************************************************************
  ...
  if (PS3.getButtonClick(R1)){
    if(a!=0){
       a = 0;
       MSS.servoWrite(0, ini0); // вниз 150
       delay(250);} 
   if(p == 0||p == 2||p == 3){
       p = 1;
      
        MSS.servoWrite(4, ini4+135);
       delay(Mytime);
       MSS.servoWrite(1, ini1-50);
       delay(Mytime);
  ...
  if (PS3.PS3Connected) { // эта проверка позволяет избежать случайных включений
    ...
    if (PS3.getAnalogHat(RightHatY) > 137)  { // вниз
     
     }


    
    if (PS3.getAnalogHat(RightHatY) < 117 ) { // вверх
...
// управление ЛЕВЫМ джойстиком
  ...
  if (PS3.getButtonClick(CROSS)) {
    ...
    if(a == 0 || a==1){
       ...
       a = 0;
       MSS.servoWrite(0,ini0); // вверх  - начальное положение
       delay(Mytime);
    }
  }
